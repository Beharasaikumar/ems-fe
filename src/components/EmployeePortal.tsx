  ...
  const PayslipsView = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex w-full sm:w-auto bg-slate-100 rounded-lg border border-slate-200 p-1 sm:shrink-0">
          <button
            onClick={() => setPayslipTab('MONTHLY')}
            className={`flex-1 sm:flex-initial min-w-0 flex items-center justify-center text-center gap-1.5 px-2 sm:px-3 py-1.5 rounded text-xs sm:text-sm font-bold leading-tight transition-all whitespace-normal sm:whitespace-nowrap ${payslipTab === 'MONTHLY' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            <FileText size={14} className="shrink-0" /> Monthly Slips
          </button>
          <button
            onClick={() => setPayslipTab('ANNUAL')}
            className={`flex-1 sm:flex-initial min-w-0 flex items-center justify-center text-center gap-1.5 px-2 sm:px-3 py-1.5 rounded text-xs sm:text-sm font-bold leading-tight transition-all whitespace-normal sm:whitespace-nowrap ${payslipTab === 'ANNUAL' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            <Award size={14} className="shrink-0" /> Annual Certificate &amp; Statement
          </button>
        </div>
      </div>
      ...
      {payslipTab === 'ANNUAL' && (
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-2.5 bg-white border border-slate-200 rounded-lg px-4 py-2 shadow-sm text-sm font-medium w-full sm:w-auto">
              <Calendar size={16} className="text-slate-400 shrink-0" />
              <div className="leading-tight min-w-0 flex-1">
                <div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Financial Year</div>
                <div className="relative">
                  <select
                    value={fyStartYear}
                    onChange={(e) => setFyStartYear(Number(e.target.value))}
                    className="w-full appearance-none font-semibold outline-none bg-transparent cursor-pointer pr-5"
                  >
                    {fyOptions.map(opt => (
