...
export const AttendanceManager: React.FC = () => {
  ...
  const confirmAndUpdate = (
    ...
    const confirmMsg = `Are you sure you want to mark ${empName} as ${status} for ${dateStr}?`;

    if (window.confirm(confirmMsg)) {
      onUpdateAttendance(empId, dateStr, status);
    }
  };
                  ...
                  <tr key={emp.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => confirmAndUpdate(emp.id, emp.name, AttendanceStatus.PRESENT)}
                          title="Present"
                        ...
                        <button
                          onClick={() => confirmAndUpdate(emp.id, emp.name, AttendanceStatus.ABSENT)}
                          title="Absent"
                          disabled={!!saving[emp.id]}
                          className={`p-2 rounded-lg transition-colors ${currentStatus === AttendanceStatus.ABSENT
                        ...
                        <button
                          onClick={() => confirmAndUpdate(emp.id, emp.name, AttendanceStatus.HALF_DAY)}
                          title="Half Day"
                          disabled={!!saving[emp.id]}
                        ...
                        <button
                          onClick={() => confirmAndUpdate(emp.id, emp.name, AttendanceStatus.LEAVE)}
                          title="Leave"
                          disabled={!!saving[emp.id]}
